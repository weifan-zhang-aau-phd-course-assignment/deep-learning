...
# Assignment: Feedforward neural network.
import torch
...
X_train = np.reshape(X_train,(no_ts,no_tf*no_qb))
# Data normalization.
uX = np.mean(X_train)
sX = np.std(X_train)
...
X_valid = np.reshape(X_valid,(no_vs,no_tf*no_qb))
...
X_test = np.reshape(X_test,(no_es,no_tf*no_qb))
X_test = (X_test - uX) / sX
...
model = Feedforward(no_tf*no_qb,128,no_cl)
model.cuda()
